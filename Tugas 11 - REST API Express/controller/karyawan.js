...
class Karyawan {
	static async register(req, res) {
		...
		let updated = { ...realData, data: [...realData.data, newUser] };
	...
	static async addSiswa(req, res) {
		...
		cariTrainer.students = [...cariTrainer.students, { ...siswa }];
		if (admin.isLogin === true){
			if(cekLogin === 1){
				await fspromise.writeFile('data.json', JSON.stringify(realData));
				res.status(201).json({ message: 'Berhasil add siswa' });
			} else {
				res.status(200).json({ message: 'Lebih dari 1 user login' });
			}
		} else {
			res.status(200).json({ message: 'Admin harus login!' });
		}
	}
}
